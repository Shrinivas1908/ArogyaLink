...
async def _get_encounter_with_patient(encounter_id: str, db: AsyncSession) -> Encounter | None:
    try:
...
async def get_fhir_bundle(
    ...
    patient_name = enc.patient.full_name if enc and enc.patient else "Ananya Sharma"
    answers = await q_engine.get_answers_dict(encounter_id, db) if enc else {
        "q_chief_complaint": ["severe_chest_pain"],
...
async def link_abha_record(
    ...
    patient_name = enc.patient.full_name if enc and enc.patient else "Ananya Sharma"
    answers = await q_engine.get_answers_dict(body.encounter_id, db) if enc else {}
